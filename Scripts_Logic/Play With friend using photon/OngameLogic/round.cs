...
public class round : MonoBehaviourPun
...
public bool player2HasStuck = false;
    ...
    // ================= INTERNAL =================
    private List<behaviour> centerCards = new List<behaviour>();
    private bool emptyCheckRunning = false;

    // FINAL RESPONSE
    private bool pendingFinalResponse = false;
    private turndown.Turn emptyPlayer;
...
public void StopDistribution()
{
    ...
    // =====================================================
    public void OnCardPlaced(behaviour card)
    {
        ...
        if (card.cardOwner == turndown.Turn.Player1)
        ...
        if (centerCards.Count >= 2)
        {
            ...
            if (last == prev)
            {
                ...
                RectTransform winnerStack =
                    card.cardOwner == turndown.Turn.Player1
                        ? player1Stack
                        : player2Stack;
    ...
    IEnumerator DelayedRedealAfterFinal()
    {
        ...
        ResetCenter();
        PlayDistribution();
        redeal.Instance.ForceRedealFromCenter();
    }
    ...
    IEnumerator CollectCenterPile(RectTransform winnerStack)
    {
        ...
        List<behaviour> pile = new List<behaviour>(centerCards);
        ResetCenter();
        pile.Reverse();

        foreach (behaviour card in pile)
        {
